...
async function load(){
 ...
 try{
  ...
  const number=p.jersey_number??p.number??'—',position=p.position||p.role||'—',division=p.division||club?.division||'—',clubName=club?.name||p.club_name||'—';
  ...
  const goals=events.filter(e=>/goal/i.test(String(e.event_type||''))).length;
  ...
  <nav class="fm-tabs"><a href="#overview">Muhtasari</a><a href="#stats">Stats</a><a href="#events">Events</a><a href="#history">Historia</a></nav>
  ...
  <section id="stats" class="card"><h2>📊 Utendaji</h2><div class="quick-grid"><div><b>${goals}</b><span>Goals</span></div><div><b>${assists}</b><span>Assists</span></div><div><b>${yellow}</b><span>Yellow</span></div><div><b>${red}</b><span>Red</span></div></div><div class="info">Takwimu zinaonyeshwa kutokana na events zilizorekodiwa na AFRN; mfumo hauundi takwimu ambazo hazipo.</div></section>
  ...
  <section id="history" class="card"><h2>📚 Historia ya Mchezaji</h2><p class="muted">Historia ya usajili, timu alizowahi kuchezea, misimu, mechi na mafanikio itaunganishwa hapa kadri data ya AFRN inavyokamilishwa.</p></section>`;
