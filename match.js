...
let liveChannel=null,refreshTimer=null,lastRendered='';
...
async function render(){
 ...
 box.innerHTML=`<section class="match-hero"><div class="match-meta">${esc(m.match_date||'Tarehe haijawekwa')} ${m.match_time?'• '+esc(m.match_time):''} ${m.venue?'• '+esc(m.venue):''}</div>${live?'<div class="live-pill">🔴 LIVE — Inasasishwa moja kwa moja</div>':''}<div class="match-teams"><div><div class="crest">⚽</div><h2>${esc(names[m.home_team_id]||m.home_team_id)}</h2><small>NYUMBANI</small></div><div class="big-score"><strong>${score}</strong><span>${esc(status||'MATOKEO')}</span></div><div><div class="crest">⚽</div><h2>${esc(names[m.away_team_id]||m.away_team_id)}</h2><small>UGENINI</small></div></div></section>
...
function startLive(){
 ...
 try{
  ...
  liveChannel=client.channel('afrn-live-match-'+matchId)
   .on('postgres_changes',{event:'*',schema:'public',table:'matches',filter:'id=eq.'+matchId},()=>render())
   .on('postgres_changes',{event:'*',schema:'public',table:'match_events',filter:'match_id=eq.'+matchId},()=>render())
   .subscribe();
