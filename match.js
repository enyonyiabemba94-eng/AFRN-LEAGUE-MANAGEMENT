...
async function load(){
 ...
 const m=matches[0], names=Object.fromEntries(clubs.map(c=>[c.id,c.name||c.short_name])), byPlayer=Object.fromEntries(players.map(p=>[p.id,p])), byClub=Object.fromEntries(clubs.map(c=>[c.id,c]));
 ...
 const starters=(clubId)=>events.filter(e=>String(e.club_id)===String(clubId)&&['starting_xi','starter','lineup_start','starting'].includes(type(e))).map(e=>byPlayer[e.player_id]).filter(Boolean);
 const subs=(clubId)=>events.filter(e=>String(e.club_id)===String(clubId)&&['substitute','bench','sub'].includes(type(e))).map(e=>byPlayer[e.player_id]).filter(Boolean);
 ...
 const hg=count(homeEvents,['goal']), ag=count(awayEvents,['goal']), hy=count(homeEvents,['yellow_card','yellow']), ay=count(awayEvents,['yellow_card','yellow']), hr=count(homeEvents,['red_card','red']), ar=count(awayEvents,['red_card','red']);
 ...
 <section id="stats" class="card"><h2>📊 Takwimu za Mchezo</h2><div class="stats-list"><div><span>⚽ Goals</span><b>${hg} — ${ag}</b></div><div><span>🟨 Yellow Cards</span><b>${hy} — ${ay}</b></div><div><span>🟥 Red Cards</span><b>${hr} — ${ar}</b></div><div><span>⏱️ Matukio</span><b>${events.length}</b></div><div><span>🔄 Substitutions</span><b>${count(homeEvents,['substitution','sub'])} — ${count(awayEvents,['substitution','sub'])}</b></div></div><p class="muted">Possession, shots, corners, xG na takwimu nyingine zitaonekana hapa tu pale zinaporekodiwa kwenye mfumo.</p></section>`;
